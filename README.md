...
> 수집부터 Airflow 일별 배치까지 완료. 리눅스 서버 배포와 Grafana 관측은 다음 단계입니다. 아래 체크리스트 참고.
...
모두 로컬 단일 노드(Docker Desktop)에서 실제로 실행해 얻은 값이며, 합성 데이터 기준입니다.

| 단계 | 측정 | 결과 |
|---|---|---|
| 검사원 | 이벤트 6,722건 중 불량 233건 판정 | 정답지 대비 precision **1.0** / recall **1.0**, 0.21초(초당 약 31,600건) |
| 적재 | 발행 3,354 = 소비 3,354 = 정상 적재 3,339 + DLQ 15 | 건수 검산 일치, DLQ 비율 0.45%(생성기가 섞은 불량 비율), 배치 커밋 포함 초당 221건 |
| 재시작 | 같은 consumer group으로 재실행 | 추가 소비 0건, 건수 변화 없음 |
| 중복 제거 | dbt staging 3,339행 → 3,235행 | **104건 제거**, 정답지 중복 수와 일치 |
...
- [ ] end-to-end 실행 스크립트
- [x] 결과 수치 (처리량, DLQ 비율, 중복 제거 정확도) — 위 "결과 수치" 표
